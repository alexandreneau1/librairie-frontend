...
export default function Dashboard() {
  ...
  const [evCategorie, setEvCategorie] = useState('')
  const [evAfficheUrl, setEvAfficheUrl] = useState('')
  const [evActif, setEvActif] = useState(true)
  ...
  const chargerEvenements = () => {
    fetch('http://localhost:3001/evenements/tous', { headers: { 'Authorization': 'Bearer ' + token() } })
      .then(r => r.json())
  ...
  const ouvrirFormulaireEvenement = (ev: Evenement | null) => {
    if (ev) {
      ...
      setEvDate(local)
      setEvCategorie(ev.categorie || '')
      setEvAfficheUrl(ev.affiche_url || '')
      setEvActif(ev.actif)
    } else {
      setEvenementEdite(null)
      setEvTitre(''); setEvDescription(''); setEvDate(''); setEvCategorie(''); setEvAfficheUrl(''); setEvActif(true)
    }
  ...
  const sauvegarderEvenement = async () => {
    ...
    if (evenementEdite) {
      await fetch(`http://localhost:3001/evenements/${evenementEdite.id}`, { method: 'PUT', headers, body })
    } else {
      await fetch('http://localhost:3001/evenements', { method: 'POST', headers, body })
    }
    setFormulaireEvenement(false)
    chargerEvenements()
  }

  const supprimerEvenement = async (id: number) => {
    if (!confirm('Supprimer cet événement ?')) return
    await fetch(`http://localhost:3001/evenements/${id}`, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + token() } })
    chargerEvenements()
  }

  const toggleActifEvenement = async (ev: Evenement) => {
    const headers = { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token() }
    await fetch(`http://localhost:3001/evenements/${ev.id}`, { method: 'PUT', headers, body: JSON.stringify({ ...ev, actif: !ev.actif }) })
    chargerEvenements()
  }
  ...
  // ── Styles ────────────────────────────────────────────────────────────────
  const styleOnglet = (nom: string) => ({ padding: '10px 20px', borderRadius: '8px', border: 'none', fontSize: '14px', fontWeight: '600' as const, cursor: 'pointer', backgroundColor: onglet === nom ? C.vert : 'transparent', color: onglet === nom ? 'white' : C.texteSecondaire })
    ...
    <div style={{ backgroundColor: C.fond, minHeight: '100vh' }}>
      <header style={{ backgroundColor: C.vert, padding: '20px 48px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
          <a href="/admin/analytics" style={{ color: C.fondAlt, fontSize: '14px', textDecoration: 'none' }}>Analytics</a>
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 24px', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', gap: '6px', marginBottom: '32px', backgroundColor: '#ede9e3', borderRadius: '10px', padding: '6px', width: 'fit-content', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setOnglet('evenements')} style={styleOnglet('evenements')}>Événements</button>
        ...
        {onglet === 'evenements' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <h2 style={{ fontSize: '20px', fontWeight: '600', color: C.texte, margin: 0 }}>
                Événements <span style={{ fontSize: '15px', fontWeight: '400', color: C.texteSecondaire }}>({evAVenir.length} à venir)</span>
              </h2>
              <button onClick={() => ouvrirFormulaireEvenement(null)} style={{ backgroundColor: C.vert, color: 'white', border: 'none', borderRadius: '8px', padding: '10px 20px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>+ Ajouter</button>
            </div>
            ...
            {formulaireEvenement && (
              <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '28px', marginBottom: '24px', boxShadow: '0 2px 10px rgba(0,0,0,0.07)', borderTop: `4px solid ${C.or}` }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                  <div style={{ gridColumn: '1 / -1' }}><label style={labelStyle}>Titre *</label><input type="text" value={evTitre} onChange={e => setEvTitre(e.target.value)} style={inputStyle} placeholder="ex. Dédicace — Jean Echenoz" /></div>
                  ...
                  <div style={{ gridColumn: '1 / -1' }}><label style={labelStyle}>URL de l'affiche</label><input type="text" value={evAfficheUrl} onChange={e => setEvAfficheUrl(e.target.value)} style={inputStyle} placeholder="https://..." /></div>
                  <div style={{ gridColumn: '1 / -1' }}><label style={labelStyle}>Description</label><textarea value={evDescription} onChange={e => setEvDescription(e.target.value)} rows={3} style={{ ...inputStyle, resize: 'vertical' as const }} placeholder="Décrivez l'événement..." /></div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <button onClick={() => setEvActif(!evActif)} style={{ padding: '8px 16px', borderRadius: '8px', border: `1px solid ${evActif?C.vert:'#ddd'}`, backgroundColor: evActif?C.fondAlt:'white', color: evActif?C.vert:C.texteSecondaire, fontSize: '13px', fontWeight: '700', cursor: 'pointer', fontFamily: 'Georgia, serif' }}>
                      {evActif ? '✓ Visible sur le site' : '○ Masqué'}
                    </button>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button onClick={sauvegarderEvenement} disabled={!evTitre || !evDate} style={{ backgroundColor: evTitre && evDate ? C.vert : '#ddd', color: 'white', border: 'none', borderRadius: '8px', padding: '10px 24px', fontSize: '14px', fontWeight: '600', cursor: evTitre && evDate ? 'pointer' : 'not-allowed' }}>
                    {evenementEdite ? 'Enregistrer' : 'Créer'}
                  </button>
                  <button onClick={() => setFormulaireEvenement(false)} style={{ backgroundColor: 'transparent', color: C.texteSecondaire, border: '1px solid #ddd', borderRadius: '8px', padding: '10px 24px', fontSize: '14px', cursor: 'pointer' }}>Annuler</button>
                </div>
              </div>
            )}
            ...
            {evAVenir.length > 0 && (
              <div style={{ marginBottom: '32px' }}>
                <p style={{ fontSize: '12px', color: C.texteSecondaire, letterSpacing: '1px', fontWeight: '600', margin: '0 0 16px' }}>À VENIR</p>
                <div style={{ backgroundColor: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.07)' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead><tr style={{ backgroundColor: '#f5f5f5' }}>{['Titre','Date','Catégorie','Affiche','Statut','Actions'].map(h => <th key={h} style={{ padding: '14px 20px', textAlign: 'left', fontSize: '13px', color: C.texteSecondaire, fontWeight: '600' }}>{h}</th>)}</tr></thead>
                    <tbody>
                      {evAVenir.map((ev, i) => (
                        <tr key={ev.id} style={{ borderTop: '1px solid #f0f0f0', backgroundColor: i%2===0?'white':'#fafafa' }}>
                          <td style={{ padding: '14px 20px', fontSize: '14px', fontWeight: '600', color: C.texte }}>{ev.titre}</td>
                          <td style={{ padding: '14px 20px', fontSize: '13px', color: C.texteSecondaire }}>{formatDateEv(ev.date_evenement)}</td>
                          <td style={{ padding: '14px 20px' }}>{ev.categorie ? <span style={{ backgroundColor: C.fondAlt, color: C.vert, padding: '3px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: '600' }}>{ev.categorie}</span> : <span style={{ color: '#ccc' }}>—</span>}</td>
                          <td style={{ padding: '14px 20px' }}>{ev.affiche_url ? <a href={ev.affiche_url} target="_blank" rel="noopener noreferrer" style={{ color: C.vert, fontSize: '12px' }}>Voir →</a> : <span style={{ color: '#ccc', fontSize: '12px' }}>—</span>}</td>
                          <td style={{ padding: '14px 20px' }}>
                            <button onClick={() => toggleActifEvenement(ev)} style={{ padding: '4px 12px', borderRadius: '20px', border: 'none', fontSize: '12px', fontWeight: '600', cursor: 'pointer', backgroundColor: ev.actif?C.fondAlt:'#f5f5f5', color: ev.actif?C.vert:'#bbb', fontFamily: 'Georgia, serif' }}>
                              {ev.actif ? '✓ Visible' : '○ Masqué'}
                            </button>
                          </td>
                          <td style={{ padding: '14px 20px' }}>
                            <div style={{ display: 'flex', gap: '8px' }}>
                              <button onClick={() => ouvrirFormulaireEvenement(ev)} style={{ backgroundColor: '#f0f0f0', color: C.texte, border: 'none', borderRadius: '6px', padding: '6px 12px', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}>Modifier</button>
                              <button onClick={() => supprimerEvenement(ev.id)} style={{ backgroundColor: 'transparent', color: '#c62828', border: '1px solid #c62828', borderRadius: '6px', padding: '6px 12px', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}>Supprimer</button>
                            </div>
                          </td>
                        </tr>
                      ))}
            ...
            {evPassés.length > 0 && (
              <div>
                <p style={{ fontSize: '12px', color: C.texteSecondaire, letterSpacing: '1px', fontWeight: '600', margin: '0 0 16px' }}>PASSÉS</p>
                <div style={{ backgroundColor: 'white', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.07)', opacity: 0.7 }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead><tr style={{ backgroundColor: '#f5f5f5' }}>{['Titre','Date','Catégorie','Actions'].map(h => <th key={h} style={{ padding: '14px 20px', textAlign: 'left', fontSize: '13px', color: C.texteSecondaire, fontWeight: '600' }}>{h}</th>)}</tr></thead>
                    <tbody>
                      {evPassés.map((ev, i) => (
                        <tr key={ev.id} style={{ borderTop: '1px solid #f0f0f0', backgroundColor: i%2===0?'white':'#fafafa' }}>
                          <td style={{ padding: '14px 20px', fontSize: '14px', color: C.texteSecondaire }}>{ev.titre}</td>
                          <td style={{ padding: '14px 20px', fontSize: '13px', color: C.texteSecondaire }}>{formatDateEv(ev.date_evenement)}</td>
                          <td style={{ padding: '14px 20px' }}>{ev.categorie ? <span style={{ backgroundColor: '#f5f5f5', color: C.texteSecondaire, padding: '3px 10px', borderRadius: '20px', fontSize: '12px' }}>{ev.categorie}</span> : <span style={{ color: '#ccc' }}>—</span>}</td>
                          <td style={{ padding: '14px 20px' }}>
                            <button onClick={() => supprimerEvenement(ev.id)} style={{ backgroundColor: 'transparent', color: '#c62828', border: '1px solid #c62828', borderRadius: '6px', padding: '6px 12px', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}>Supprimer</button>
                          </td>
                        </tr>
            ...
            {evenements.length === 0 && !formulaireEvenement && (
              <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '60px', textAlign: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                ...
                <p style={{ color: C.texteSecondaire, fontSize: '15px', margin: '0 0 20px' }}>Aucun événement programmé</p>
                <button onClick={() => ouvrirFormulaireEvenement(null)} style={{ backgroundColor: C.vert, color: 'white', border: 'none', borderRadius: '8px', padding: '10px 20px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>+ Premier événement</button>
              </div>
            )}
          </>
        )}
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px' }}>
              <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', borderTop: '4px solid ' + C.vert }}>
                ...
                <button onClick={lancerImportCatalogue} disabled={!fichierCatalogue || importEnCours === 'catalogue'} style={{ width: '100%', padding: '12px', backgroundColor: fichierCatalogue ? C.vert : '#ddd', color: 'white', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: fichierCatalogue ? 'pointer' : 'not-allowed', fontFamily: 'Georgia, serif' }}>
                  {importEnCours === 'catalogue' ? '⏳ Import...' : 'Lancer l\'import'}
                </button>
              </div>
              <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', borderTop: '4px solid ' + C.or }}>
                ...
                <button onClick={lancerScrapingBabelio} disabled={scrapingEnCours} style={{ width: '100%', padding: '12px', backgroundColor: scrapingEnCours ? '#ddd' : C.orIntense, color: 'white', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: scrapingEnCours ? 'not-allowed' : 'pointer', fontFamily: 'Georgia, serif' }}>
                  {scrapingEnCours ? '⏳ Récupération...' : 'Mettre à jour le top ventes'}
                </button>
              </div>
              <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', borderTop: '4px solid #8B4513' }}>
                ...
                <button onClick={lancerImportPrix} disabled={!fichierPrix || importEnCours === 'prix'} style={{ width: '100%', padding: '12px', backgroundColor: fichierPrix ? '#8B4513' : '#ddd', color: 'white', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: fichierPrix ? 'pointer' : 'not-allowed', fontFamily: 'Georgia, serif' }}>
                  {importEnCours === 'prix' ? '⏳ Import...' : 'Importer les prix'}
                </button>
              </div>
            </div>
          </>
