      ...
      <main style={{ maxWidth: '800px', margin: '0 auto', padding: '48px 24px 80px', boxSizing: 'border-box' }}>
          ...
          {(['panier', 'coordonnees', 'confirmation'] as Etape[]).map((e, i) => {
            ...
            return (
              <div key={e} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontSize: '14px', fontWeight: actif ? '700' : '400', color: actif ? C.texte : C.texteSecondaire }}>{labels[e]}</span>
        ...
        {etape === 'panier' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '24px' }}>
              ...
              {panier.length > 0 && (
                <button onClick={() => { if (confirm('Vider le panier ?')) { viderPanier(); syncPanier() } }} style={{ background: 'none', border: 'none', color: '#c0392b', fontSize: '13px', cursor: 'pointer', fontFamily: 'Georgia, serif', textDecoration: 'underline' }}>
                  Vider le panier
                </button>
              )}
            </div>
              ...
              <>
                  ...
                  {panier.map(article => (
                    <div key={article.livre_id} style={{ backgroundColor: 'white', borderRadius: '12px', padding: '20px 24px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', gap: '20px', borderLeft: `4px solid ${C.vert}` }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <button onClick={() => handleQuantite(article.livre_id, article.quantite - 1)} style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1px solid #ddd', backgroundColor: 'white', cursor: 'pointer', fontSize: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.texteSecondaire }}>−</button>
                        <span style={{ width: '32px', textAlign: 'center', fontWeight: '700', fontSize: '16px', color: C.texte }}>{article.quantite}</span>
                        <button onClick={() => handleQuantite(article.livre_id, article.quantite + 1)} style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1px solid #ddd', backgroundColor: 'white', cursor: 'pointer', fontSize: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.texteSecondaire }}>+</button>
                      </div>
                      <div style={{ textAlign: 'right', minWidth: '80px' }}>
                        <p style={{ fontSize: '18px', fontWeight: '700', color: C.vert, margin: '0 0 2px' }}>{(article.prix * article.quantite).toFixed(2)} €</p>
                        {article.quantite > 1 && <p style={{ fontSize: '12px', color: '#bbb', margin: 0 }}>{article.prix} € × {article.quantite}</p>}
                      </div>
                      <button onClick={() => handleRetirer(article.livre_id)} style={{ padding: '6px 10px', backgroundColor: '#fff0f0', border: 'none', borderRadius: '8px', color: '#c0392b', cursor: 'pointer', fontSize: '16px' }}>✕</button>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', marginBottom: '24px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', paddingBottom: '16px', borderBottom: '1px solid #eee' }}>
                    <span style={{ fontSize: '14px', color: C.texteSecondaire }}>{panier.reduce((acc, a) => acc + a.quantite, 0)} exemplaire{panier.reduce((acc, a) => acc + a.quantite, 0) > 1 ? 's' : ''}</span>
                ...
                {/* Si client connecté : recap infos + bouton direct */}
                {clientConnecte && clientInfo ? (
                  <div>
                    <div style={{ backgroundColor: C.fondAlt, borderRadius: '12px', padding: '20px 24px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <p style={{ fontSize: '12px', color: C.texteSecondaire, letterSpacing: '1px', margin: '0 0 6px', fontWeight: '600' }}>RÉSERVATION AU NOM DE</p>
                        <p style={{ fontSize: '16px', fontWeight: '700', color: C.texte, margin: '0 0 2px' }}>{clientInfo.prenom} {clientInfo.nom}</p>
                        <p style={{ fontSize: '14px', color: C.texteSecondaire, margin: 0 }}>{clientInfo.email}</p>
                      </div>
                      <a href="/compte/dashboard" style={{ fontSize: '13px', color: C.texteSecondaire, textDecoration: 'underline' }}>Modifier</a>
                    </div>
                    ...
                    <button
                      onClick={handleCommanderConnecte}
                      disabled={chargement}
                ...
                <a href="/livres" style={{ display: 'block', textAlign: 'center', marginTop: '16px', color: C.texteSecondaire, fontSize: '14px', textDecoration: 'none' }}>← Continuer mes achats</a>
              </>
            )}
          </>
        )}

        {/* ── ÉTAPE 2 : COORDONNÉES (non connecté seulement) ───────────── */}
        {etape === 'coordonnees' && (
          <>
            <h1 style={{ fontSize: '28px', fontWeight: '700', color: C.texte, margin: '0 0 8px' }}>Vos coordonnées</h1>
            <p style={{ color: C.texteSecondaire, fontSize: '14px', marginBottom: '32px' }}>Pour vous prévenir quand votre réservation est prête à retirer.</p>
            ...
            <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '32px', boxShadow: '0 2px 16px rgba(0,0,0,0.06)', marginBottom: '24px' }}>
              <div style={{ marginBottom: '20px' }}>
                <label style={{ display: 'block', fontSize: '13px', color: C.texteSecondaire, marginBottom: '8px', fontWeight: '600' }}>Nom complet *</label>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <p style={{ fontSize: '12px', color: '#bbb', margin: '6px 0 0' }}>Vous recevrez un email quand votre réservation sera prête.</p>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '13px', color: C.texteSecondaire, marginBottom: '8px', fontWeight: '600' }}>Téléphone <span style={{ fontWeight: '400', color: '#bbb' }}>(optionnel)</span></label>
            ...
            <button onClick={handleValider} disabled={chargement} style={{ width: '100%', padding: '16px', backgroundColor: chargement ? '#aaa' : C.orIntense, color: 'white', border: 'none', borderRadius: '12px', fontSize: '16px', fontWeight: '700', cursor: chargement ? 'not-allowed' : 'pointer', fontFamily: 'Georgia, serif', marginBottom: '12px' }}>
              {chargement ? 'Envoi en cours...' : 'Confirmer la réservation — Paiement en boutique'}
            </button>
            <button onClick={() => { setEtape('panier'); setErreur('') }} style={{ width: '100%', padding: '12px', backgroundColor: 'transparent', color: C.texteSecondaire, border: '1px solid #ddd', borderRadius: '12px', fontSize: '14px', cursor: 'pointer', fontFamily: 'Georgia, serif' }}>
              ← Retour au panier
            </button>
          </>
        )}
        ...
        {etape === 'confirmation' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h1 style={{ fontSize: '28px', fontWeight: '700', color: C.texte, margin: '0 0 12px' }}>Réservation confirmée !</h1>
            <p style={{ color: C.texteSecondaire, fontSize: '15px', marginBottom: '32px', lineHeight: '1.7' }}>
              ...
              Nous vous préviendrons dès que votre réservation sera prête à retirer.
            </p>
            ...
            {recap.length > 0 && (
              <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '28px', boxShadow: '0 2px 16px rgba(0,0,0,0.06)', marginBottom: '32px', textAlign: 'left' }}>
                <p style={{ fontSize: '12px', color: C.texteSecondaire, letterSpacing: '1px', margin: '0 0 16px', fontWeight: '600' }}>VOTRE RÉSERVATION</p>
                {recap.map((l, i) => (
            ...
            <div style={{ backgroundColor: C.fondAlt, borderRadius: '12px', padding: '20px 24px', marginBottom: '32px', textAlign: 'left' }}>
              <p style={{ fontWeight: '700', color: C.vert, margin: '0 0 8px', fontSize: '15px' }}>📍 Où retirer votre réservation ?</p>
